...
void CP_example()
{
	...
	if (MC == MC1)
	{
		...
		/* Receive Functions */
		CP_InitRx(&DataObj_MC2, sizeof(DataObj_MC2), 0x001);
		CP_StartRx(0x001);
		CP_StatusRx(0x001);
		CP_DeleteRx(0x001);

		/* Transmit Functions */
		CP_InitTx(&DataObj_MC1, sizeof(DataObj_MC1), 0x001);
		CP_StartTx();
		CP_StatusTx();

		CP_ERROR_DESCRIPTION[CP_LastErrorCodes[0]];
		CP_ERROR_DESCRIPTION[CP_LastErrorCodes[1]];
		CP_ERROR_DESCRIPTION[CP_LastErrorCodes[2]];
